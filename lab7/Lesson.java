...
public class Lesson implements VisitableTimetableElement, Observable,Comparable1<Lesson>{
    private Term term;
    private String name;
    ...
    @Override
    public void notifyObservers(Term term1, Term term2) {
        int n=observers.size();
        for(int i=0;i<n;i++){
            observers.get(i).update(term1,term2);
        }
    }
    @Override
    public int compareTo1(Lesson lesson){
        if (this.term.getday().ordinal()<lesson.getterm().getday().ordinal()) return -1;
        else if(this.term.getday().ordinal()==lesson.getterm().getday().ordinal()){
            if(this.term.gethour()<lesson.getterm().gethour()) return -1;
            else if(this.term.gethour()==lesson.getterm().gethour()){
                if(this.term.getminute()<lesson.getterm().getminute()) return -1;
                else if(this.term.getminute()==lesson.getterm().getminute()) return 0;
                else return 1;
            }
            else return 1;
        }
    ...
    public Timetable2 getTimetable(){
    ...
    public Lesson(Timetable2 timetable, Term term, String name, String teacherName, int year) {
        this.timetable=timetable;
        this.term=term;
        this.name=name;
        this.teacherName=teacherName;
